...
function resolveColumnColorHex(norm, settings, normProject, normMaybeSomeday, normActive, generalHex, hueHex, baseL) {
...
function buildConfig(settings) {
  ...
  const normActive = (settings.activeColumns && settings.activeColumns.length ? settings.activeColumns : ["#next", "#important", "#today"]).map(normalizeTag).filter((t) => !normMaybeSomeday.includes(t));
...
function findArchivedTitleLine(lines, calloutIdx, plainTitle, indent) {
  if (calloutIdx < 0 || !plainTitle)
    return -1;
  for (let i = calloutIdx + 1; i < lines.length; i++) {
    ...
    if (parsed.indent === indent && parsed.text.trim() === plainTitle)
      return i;
  }
  return -1;
}
async function archiveToSection(app, filePath, mainLineNum, subLines, config, _isTopLevel = true, tickMain = true, keepRecurring = true) {
  try {
    ...
    if (_isTopLevel && !hasRecurrentInBlock) {
      ...
      const existingTitleIdx = calloutIdx >= 0 ? findArchivedTitleLine(lines, calloutIdx, mainParsedForMatch.text.trim(), mainParsedForMatch.indent) : -1;
...
function wireSubtaskTree(app, containerEl, titleEl, root, config, onEditSubtask, onDeleteSubtask, onChange, setEscapeHandler, dialogEscapeDefault) {
  ...
  const nodeIsArchived = (n) => parseTaskLine(n.raw).tags.some(isArchivedTag);
  ...
  const startNewNodeEdit = (node, isCheckboxNode) => {
  ...
  const addNode = (isCheckboxNode) => {
    const node = { id: nextNewId--, raw: isCheckboxNode ? "- [ ] " : "- ", trailingRaw: [], children: [] };
...
function showCardColorDialog(app, existingColor, title, cardLineNum, subtaskTree, config, existingUncounted, existingUncountedChildren, onApply, onSetUncountedFlags, onReorder, onDelete, onEditSubtask, onDeleteSubtask) {
  ...
  const showArchivedCheckbox = dialog.querySelector("#k-subtask-show-archived");
  const showArchivedTrack = dialog.querySelector("#k-subtask-show-archived-track");
  const showArchivedThumb = dialog.querySelector("#k-subtask-show-archived-thumb");
  ...
  const applyShowArchivedVisual = () => {
    if (!showArchivedCheckbox || !showArchivedTrack || !showArchivedThumb)
      return;
...
async function stampMissingCreatedDates(app, paths, config) {
  ...
  const visit = (filePath, node, isSubtask) => {
    ...
    for (const sub of node.subs ?? [])
      visit(filePath, sub, true);
  };
  for (const card of items)
    visit(card.filePath, card.item, false);
...
var KanbanView = class extends import_obsidian2.ItemView {
  ...
  scrollPastSearchBar() {
    const scroll = this.contentEl.querySelector("#kanban-scroll");
    if (!scroll)
      return;
...
function isCheckedItemText(text) {
  return /^[-*+]\s+\[[xX]\]/.test((text ?? "").trim());
}
function columnLabel(tag) {
  return tag.replace(/^#/, "").replace(/\b\w/g, (l) => l.toUpperCase());
}
function nodeColumnLabel(tags, config) {
...
function renderLineChart(parent, doc, buckets, series, yMaxOverride, zeroLineColor) {
  ...
  tooltip.style.cssText = "position:absolute;pointer-events:none;background:var(--background-primary);border:1px solid var(--background-modifier-border);border-radius:6px;padding:6px 9px;font-size:.8em;color:var(--kb-text);box-shadow:0 2px 6px rgba(0,0,0,.15);display:none;white-space:nowrap;z-index:5;";
  const move = (clientX) => {
...
var KanbanSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  async display() {
    ...
    try {
      ...
      typeGroup((box) => {
        new import_obsidian4.Setting(box).setName("Maybe Someday columns").setDesc(
          "Comma-separated tags for one or more Maybe Someday columns. Cards here are never treated as Active (see 'Active columns' below), and don't get a creation date stamped until they're moved out to another column \u2014 a card that already had a creation date before landing here keeps it."
        ).addText(
          (text) => text.setPlaceholder("#maybesomeday").setValue((this.plugin.settings.maybeSomedayColumns || []).join(", ")).onChange(async (value) => {
            this.plugin.settings.maybeSomedayColumns = value.split(",").map((t) => t.trim()).filter(Boolean);
            await this.plugin.saveSettings();
          })
        );
        plainSlider(
          ...
          "Maybe Someday columns have no Hue of their own \u2014 like Non-active columns, they use Column background's Hue, shifted by this offset instead (one shared value covering every Maybe Someday column). Added to the general Lightness. Negative darkens, positive lightens, 0 = same as Active.",
          () => s.maybeSomedayLightnessDelta,
          (v) => {
            s.maybeSomedayLightnessDelta = v;
          },
          "kb-lightness",
          [-15, 15]
        );
      ...
      typeGroup((box) => {
        new import_obsidian4.Setting(box).setName("Active columns").setDesc(
          "Comma-separated tags for columns considered 'active' work. A project card is highlighted as unmanaged work only when none of its sub-tasks are in one of these columns, and not all of its sub-tasks are in the Later or Recurrent columns. The Maybe Someday column is always excluded here, even if listed \u2014 it can never be Active."
        ).addText((text) => {
